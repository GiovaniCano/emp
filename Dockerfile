...
RUN docker-php-ext-configure exif --enable-exif
    # exif for laravel-sitemap
RUN docker-php-ext-configure pcntl --enable-pcntl
    # pcntl for queues timeout https://laravel.com/docs/10.x/queues#timeout
RUN docker-php-ext-install \
    pdo_mysql bcmath gd exif pcntl
...
CMD ["bash", "-c", " \
        echo q | sudo -S service supervisor start && \
        echo q | sudo -S supervisorctl start laravel-worker:* && \
        echo q | sudo -S apache2ctl -D FOREGROUND \
    "]
...
# docker exec laravel composer create-project laravel/laravel .
